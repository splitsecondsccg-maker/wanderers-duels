...
function noteHopeHeal(g,u,h){
  if(!g || !u || u.dead || h<=0) return;
  const hopes=alive(g,u.side).filter(x=>x.id==="hope");
  if(!hopes.length) return;
  g.hopeHealedAllies = g.hopeHealedAllies || {};
  const list = g.hopeHealedAllies[u.side] || (g.hopeHealedAllies[u.side]=[]);
  if(list.includes(u.id)) return;
  list.push(u.id);
  for(const hope of hopes) hope.buff.hopeDamage = list.length;
}
function heal(u,n,g=null){
...
function damage(g,src,t,amount,opt={}){
  ...
  if(src.id==="hope" && opt.attack) raw += src.buff?.hopeDamage || 0;
  ...
  if(reduced>0){
    ...
    if(src.id==="eva" && (t.status.bleed||0)>0) heal(src,1,g);
    killCheck(t);
  }
  return reduced;
}
function triggerZahriaAegis(g,victim){
  if(!g || !victim) return;
  const side = victim.side==="A" ? "B" : "A";
  const zahrias=alive(g,side).filter(u=>u.id==="zahria");
  if(!zahrias.length) return;
  g.zahriaBleedVictims = g.zahriaBleedVictims || {};
  const list = g.zahriaBleedVictims[side] || (g.zahriaBleedVictims[side]=[]);
  if(list.includes(victim.id)) return;
  list.push(victim.id);
  for(const z of zahrias) z.tempArmor += 1;
}

function validTargets(g,u,a){
  if(a.kind==="poomMassGuardMind") return [u];
  if(a.kind==="dodge" || a.kind==="selfCounter") return [u];
  if(a.kind==="allStatus" || a.kind==="massDrainBleed") return [null];
  if(a.kind==="protect" || a.kind==="bloodWard" || a.kind==="poisonHands" || a.kind==="frontDamageForAllyHp" || a.kind==="frontStatusForAllyHp" || a.kind==="empowerNextAttack" || a.kind==="frostArmorRetaliate" || a.kind==="singleHeal" || a.kind==="grantShield") return allies(g,u);
  const es=enemies(g,u);
  if(["rowStatus","rowDamageStatus","rowMultiStatus","rowBleedAmplify"].includes(a.kind)) return es; // target picks row
...
function isSetup(a){
  return ["status","multiStatus","rowStatus","rowMultiStatus","rowDamageStatus","attackSetup","damageStatus","damageStatusOnHit","freezeSetup","bloodWard","poisonHands","frontStatusForAllyHp","selfAllStatus","allStatus","rowBleedAmplify","empowerNextAttack","whiteout","frontHypno","frostArmorRetaliate","proliferate"].includes(a.kind);
}
function isPayoff(a){
...
function approxDamage(a,t,g,u){
  ...
  switch(a.kind){
    ...
    case "freezePayoff": d=(a.dmg||3)+(t.status.freeze||0)*2; break;
    ...
    case "armorStrike": d=(a.dmg??2)+armor(u); break;
...
function applyAbility(g,u,a,t){
  ...
  switch(a.kind){
    ...
    case "drain": { const dealt=damage(g,u,t,a.dmg,{attack:true,melee:a.range==="melee"}); if(dealt>0) heal(u,a.heal,g); break; }
    case "armorStrike": damage(g,u,t,(a.dmg??2)+armor(u),{attack:true,melee:true}); break;
    case "singleHeal": heal(t,a.heal||5,g); break;
    case "grantShield": if(t) t.shield=(t.shield||0)+(a.shield||5); break;
    case "delayedAttack":
      g.delayedActions = g.delayedActions || [];
      if(t) g.delayedActions.push({sourceId:u.id, sourceSide:u.side, targetId:t.id, dueRound:g.round+(a.delay||1), dmg:a.dmg||6});
      break;
    ...
    case "allStatus": for(const x of enemies(g,u)) addStatus(g,x,a.status,a.stacks,u); break;
    case "rowBleedAmplify": for(const x of rowUnits(g,t.side,t.row||"front")) if(x.status.bleed) addStatus(g,x,"bleed",a.stacks||2,u); break;
    case "massDrainBleed": {
      let total=0;
      for(const x of enemies(g,u)){
        const b=x.status.bleed||0;
        if(!b) continue;
        x.status.bleed=0;
        const before=x.hp;
        lifeLoss(g,x,b,u);
        const lost=Math.max(0,before-x.hp);
        total+=lost;
        if(lost>0) triggerZahriaAegis(g,x);
      }
      if(total>0) heal(u,total,g);
      break;
    }
    ...
    case "bleedPayoff": { const b=t.status.bleed||0; t.status.bleed=0; const dealt=damage(g,u,t,b*(a.mult||1)+(a.bonus||0),{attack:true,melee:a.range==="melee"}); if(b>0) heal(u,a.heal||0,g); break; }
    ...
    case "freezePayoff": { const f=t.status.freeze||0; t.status.freeze=0; damage(g,u,t,(a.dmg||3)+f*2,{attack:true,pierce:a.pierce||0}); break; }
    ...
    case "revenge": { const wasAttacked=!!u.buff.wasAttacked; lifeLoss(g,u,a.self||2,u); damage(g,u,t,a.dmg+(wasAttacked?(a.bonus||3):0),{attack:true,melee:true}); break; }
...
function startRoundHopeState(g){
  g.hopeHealedAllies = {};
  for(const u of g.units) if(u.id==="hope") u.buff.hopeDamage = 0;
  g.zahriaBleedVictims = {};
}
function resolveDelayedActions(g){
  const queue = g.delayedActions || [];
  if(!queue.length) return;
  const ready = queue.filter(x=>x.dueRound <= g.round);
  g.delayedActions = queue.filter(x=>x.dueRound > g.round);
  for(const item of ready){
    const src = g.units.find(u=>u.id===item.sourceId && u.side===item.sourceSide && !u.dead);
    const t = g.units.find(u=>u.id===item.targetId && !u.dead);
    if(src && t) damage(g,src,t,item.dmg||6,{attack:true});
  }
}
...
function expectedRawDamageSmartAI(a,t,g,u){
  ...
  if(a.kind==="delayedAttack") d = a.dmg || 6;
  if(a.kind==="massDrainBleed") d = enemies(g,u).reduce((s,x)=>s+(x.status.bleed||0),0);
...
function statusPotentialSmartAI(a,t,g,u){
  ...
  if(a.kind==="allStatus") enemies(g,u).forEach(x=>add(a.status,a.stacks||1,x));
  if(a.kind==="rowBleedAmplify") enemies(g,u).filter(x=>x.row===(t?.row||"front") && x.status.bleed).forEach(x=>add("bleed",a.stacks||2,x));
  return v;
}
function targetScoreSmartAI(g,u,a,t,ap){
  if(!t) {
    if(a.kind==="poomMassGuardMind") return payoffReady(a,t,g,u)?30:-40;
    if(a.kind==="allStatus") return 13 + enemies(g,u).filter(x=>!(x.status?.[a.status]||0)).length * 4;
    if(a.kind==="massDrainBleed") return enemies(g,u).reduce((s,x)=>s+(x.status.bleed||0),0) * 2.4;
    return 0;
  }
  if(t.side===u.side){
    let s=4;
    if(a.kind==="singleHeal"){
      const missing=Math.max(0,(t.maxHp||t.hp)-t.hp);
      s += Math.min(a.heal||5,missing)*3 + (t.hp<=t.maxHp*.45 ? 14 : 0);
    }
    if(a.kind==="grantShield") s += (a.shield||5)*1.4 + (t.hp<=t.maxHp*.55 ? 12 : 0);
    if(a.guard) s += t.hp<=t.maxHp*.45 ? 20 : 2;
    if(["protect","bloodWard","grantShield"].includes(a.kind)) s += t.hp<=t.maxHp*.55 ? 16 : -4;
    if(["poisonHands","empowerNextAttack"].includes(a.kind)) s += 15 + (t.planned ? -8 : 0);
    if(t.id===u.id && a.range==="ally") s -= 8;
    return s;
  }
...
function abilityScoreSmartAI(g,u,a,ts,ap,plans){
  ...
  if(a.guard){
    s += lowSelf ? 10 : -3;
    s += lowAlly ? 8 : 0;
    if(["protect","bloodWard","grantShield"].includes(a.kind)) s += lowAlly ? 8 : -8;
...
function simulateGame(gameId){
  ...
  const g={id:gameId, round:1, units:[...makeTeam("A",idsA),...makeTeam("B",idsB)], abilityUses:{}, delayedActions:[], hopeHealedAllies:{}};
  const maxRounds=20;
  for(; g.round<=maxRounds; g.round++){
    startRoundHopeState(g);
    resolveDelayedActions(g);
    if(!alive(g,"A").length || !alive(g,"B").length) break;
    const plans=[...choosePlans(g,"A"), ...choosePlans(g,"B")];
    ...
    for(const p of plans){
      ...
      if(!p.t && !["poomMassGuardMind","dodge","selfCounter","allStatus","massDrainBleed"].includes(p.a.kind)) continue;
