...
const POLICY_A = arg("policyA", "old");
const POLICY_B = arg("policyB", "old");
...
function simulateGame(gameId){
  ...
  for(; g.round<=maxRounds; g.round++){
    const plans=[
      ...(POLICY_A==="smart"?choosePlansSmartAI(g,"A"):choosePlansOldAI(g,"A")),
      ...(POLICY_B==="smart"?choosePlansSmartAI(g,"B"):choosePlansOldAI(g,"B"))
    ];
...
const policyCharStats={};
const hpMarginStats={wins:0,totalMargin:0,totalWinnerHp:0,totalLoserHp:0};
function ensurePolicyChar(name,pilot,opp){
  const key=`${name}|${pilot}|${opp}`;
  policyCharStats[key] ||= {character:name,pilotPolicy:pilot,opponentPolicy:opp,picks:0,wins:0,damage:0,taken:0,guards:0};
  return policyCharStats[key];
}
...
for(let i=0;i<GAMES;i++){
  ...
  for(const u of g.units){
    ...
    const pilot = u.side==="A" ? POLICY_A : POLICY_B;
    const opp = u.side==="A" ? POLICY_B : POLICY_A;
    const ps = ensurePolicyChar(u.name,pilot,opp);
    ps.picks++;
    if(u.side===winner) ps.wins++;
    ps.damage += u.stats.damage;
    ps.taken += u.stats.taken;
    ps.guards += u.stats.guards;
  }
...
const policyCharacterSummary=Object.values(policyCharStats).map(s=>({
  character:s.character,pilotPolicy:s.pilotPolicy,opponentPolicy:s.opponentPolicy,picks:s.picks,
  winRate:pct(s.wins/Math.max(1,s.picks)),
  avgDamage:+(s.damage/Math.max(1,s.picks)).toFixed(2),
  avgTaken:+(s.taken/Math.max(1,s.picks)).toFixed(2),
  avgGuards:+(s.guards/Math.max(1,s.picks)).toFixed(2)
})).sort((a,b)=>a.character.localeCompare(b.character)||a.pilotPolicy.localeCompare(b.pilotPolicy)||a.opponentPolicy.localeCompare(b.opponentPolicy));
const report={
  meta:{games:GAMES, seed:SEED, policyA:POLICY_A, policyB:POLICY_B, avgRounds:+(totalRounds/GAMES).toFixed(2), generatedAt:new Date().toISOString()},
  notes:[
